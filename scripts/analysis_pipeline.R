...
# Table 1: Ratings by block and order ----
table1 <- questionnaire %>% 
  dplyr::group_by(condition,question) %>% 
  summarise(median= round(median(value),2),mean = round(mean(value),2), sd =round(sd(value),2))
...
order_table1 <- questionnaire %>% filter(order != 0) %>%
  dplyr::group_by(condition,question,order) %>% 
  summarise(median= round(median(value),2),mean = round(mean(value),2),sd =round(sd(value),2))
...
# descriptive suplementary table S4
...
# Supplementary Table S6: geomean F0 pre & post per condition
...
# Figure 4: N1 amplitude by condition ----
...
# Supplementary Figure S7: N1 amplitude by group ----
...
# Table 2: N1 suppression descriptives ----
table5_all <- N1SIS %>% 
...
# Figure 5: N1 suppression by group ----
# figure 5 a1) SPQ
stat.test <- N1SIS %>%
  group_by(condition) %>%
...
p5_SPQ <- ggbarplot(table5_SPQ, x = "condition", y = "mean_p", 
...
p5_PDI <- ggbarplot(table5_PDI, x = "condition", y = "mean_p", 
  fill = "sample_PDI", 
...
# Fig 5 b1) SPQ 
p51_SPQ <- ggplot(match_data, aes(x = F0shift, y = N1, color = sample_SPQ, shape = sample_SPQ, linetype = sample_SPQ)) +
...
# Fig 5 b2) PDI 
p51_PDI <- ggplot(match_data, aes(x = F0shift, y = N1, color = sample_PDI, shape = sample_PDI, linetype = sample_PDI)) +
  geom_point(alpha = 0.8, size = 2) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 1, alpha = 0.2) +
